...
function Builder({user , setUser}) {
  ...
  <script src="${CLIENT_URL}/assistant.js" data-user-id="${user?._id}"></script>
              ...
              <button onClick={()=>setEditAssistant(true)} className='mt-6 h-12 px-6 rounded-2xl bg-linear-to-r from-pink-600 to-orange-500 text-white font-medium'>Edit Assistant</button>
        ...
        {editAssistant && <div className='space-y-6'>
                ...
                {THEMES.map((item)=>(
                  ...
                  onClick={()=>setTheme(item)}
                   className={`py-3 rounded-2xl border-2 capitalize ${theme === item
                        ? "border-orange-500 bg-pink-50 text-orange-700"
                        : "border-gray-200"
                        }`}>{item}
                  </button>
                ))}
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            <div className='flex items-center justify-between mb-5 gap-4 flex-wrap'>
              ...
              className='px-4 py-2 rounded-xl bg-linear-to-r from-pink-600 to-orange-500 text-white text-sm font-medium hover:scale-[1.02] transition-all cursor-pointer'>
                Get API KEY
              </a>
            </div>
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
            <div className='flex items-center justify-between mb-5 flex-wrap'>
              ...
              <button onClick={addPage} className='flex items-center gap-2 px-4 py-2 rounded-xl bg-linear-to-r from-pink-600 to-orange-500 text-white text-sm'>
                <FiPlus/>Add
              </button>
            </div>
        ...
        !businessType ||
        !businessDescription ||
        !geminiApiKey} className='w-full h-14 rounded-2xl bg-linear-to-r from-pink-600 to-orange-500 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed'>
            {
